...
class DocuwareOperations(models.Model):
                    ...
                    if done:
                        ...
                        if signants:
                            ...
                            if not document.docuware_document_error_log:
                                document.docuware_document_error_log = str(datetime.now()) + " " + \
    ...
    def call_viafirma_nominas(self):
        ...
        nominas = self.env['docuware.documents'].search([('stage_id', '=', ready)])
        for nomina in nominas:
            try:
                ...
                if not nomina.docuware_document_error_log:
                    nomina.docuware_document_error_log = str(datetime.now()) + " " + str(e) + "\n"
                else:
                    nomina.docuware_document_error_log += str(datetime.now()) + " " + str(e) + "\n"
    ...
    def get_signed_nominas(self):
        ...
        nominas = self.env['docuware.documents'].search([('stage_id', '=', processing)], limit=1)
