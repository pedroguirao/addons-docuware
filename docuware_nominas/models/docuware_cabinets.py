...
class DocuwareCabinets(models.Model):
    ...
    def get_nominas_data(self):
        ...
        cabinets = self.env['docuware.cabinet'].search([('cabinet_type', '=', 'nominas')])
        for cabinet in cabinets:
            for document in cabinet.cabinet_document_ids:
                if document.docuware_nomina_done:
                    print("Not done")
                    done = document.get_document_data_from_operation(cabinet.mandatory_field_ids, s)
                    try:
                        if done:
                            print("DONE")
                            ...
                            if signants:
                                ...
                                viafirma = document.viafirma_id = self.env['viafirma'].create({
                                    ...
                                    'template_id': cabinet.viafirma_template.id,
                                    'notification_type_ids': [(6, 0,cabinet.viafirma_notifications.ids)],
